import {
  ...
  const ScheduleTemplate: React.FC<RouteComponentProps<any, any, LocationState>>  = ({location}) => {
    ...
    if(temp?.Length < 60){
        duration = convertMillisecondsToHoursAndMinutes(parseInt(temp?.Length, 10)).minutes + ' minute(s)';
    } else {
        duration = convertMillisecondsToHoursAndMinutes(parseInt(temp?.Length, 10)).hours + ' hour(s) ' + convertMillisecondsToHoursAndMinutes(parseInt(temp?.Length, 10)).minutes + ' minute(s)'
    }
    ...
    const [subject, setSubject] = useState<string>(temp?.TemplateName || '');
    const [date, setDate] = useState<string>('');
    const [timeValue, setTimeValue] = useState<string>('');
    const [durationValue, setDurationValue] = useState<string>(
        temp?.Length 
        ? convertMillisecondsToHoursAndMinutes(temp?.Length).hours + ' hour(s)' + convertMillisecondsToHoursAndMinutes(temp?.Length).minutes + ' minute(s)'
        : ''
    );
    const [participantNum, setParticipantNum] = useState<string>(temp?.Parties || '');
